...
@section('content')
<!-- #PERUBAHAN: UI Hero Section diperbarui dengan gradient background dan tampilan responsif -->
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="flex flex-col space-y-4">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                ...
                <!-- #DITAMBAHKAN: Tombol untuk menambahkan competition experience di profil user -->
...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 pb-10">
    <!-- #PERUBAHAN: Kategori filter disesuaikan dengan standar kategori aplikasi -->
    <div class="bg-white rounded-xl shadow-md mb-8 overflow-hidden">
        <div class="p-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="sm:flex-shrink-0">
                    <form action="{{ route('competitions.random-members', $competition->id) }}" method="GET" class="flex items-center space-x-2">
                        <!-- #PERUBAHAN: Daftar kategori disesuaikan (Desain, Teknologi, Musik, Olahraga, Pendidikan, Other) -->
            ...
            <!-- #DITAMBAHKAN: Alert untuk menampilkan status filter aktif -->
            @if(request('category'))
    ...
    <!-- Team Formation Info Section: DIPERBARUI - Tampilan informasi tim yang lebih modern -->
    <div class="bg-white rounded-xl shadow-md mb-8 overflow-hidden">
        <div class="p-6">
    ...
    <!-- Potential Team Members Section: DIPERBARUI - Tampilan kartu anggota dengan penanda pengalaman -->
    <!-- #DITAMBAHKAN: Logic untuk memfilter user berdasarkan kategori pengalaman yang dipilih -->
    @php
        if(request('category')) {
            // Jika ada filter kategori, tampilkan hanya user dengan pengalaman yang sesuai
            ...
            // Jika tidak ada filter, tampilkan semua user
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
        ...
        @foreach($filteredUsers as $user)
            <!-- #DITAMBAHKAN: Persiapan data untuk tampilan member, termasuk pengecekan kecocokan kategori -->
            @php
            ...
            @endphp
            <!-- #DITAMBAHKAN: Border hijau untuk kartu anggota yang memiliki pengalaman sesuai kategori filter -->
            <!-- #DITAMBAHKAN: Highlight visual dengan background hijau untuk anggota yang memiliki pengalaman yang cocok -->
            <div class="{{ $hasRequestedCategory ? 'bg-green-50 ring-2 ring-green-500' : 'bg-white' }} rounded-xl shadow-md overflow-hidden transform transition-all duration-300 hover:-translate-y-2 hover:shadow-lg">
                <!-- #DITAMBAHKAN: Banner "Recommended Match" untuk anggota tim yang cocok dengan filter -->
                ...
                <div class="p-6">
                    ...
                    <!-- #DITAMBAHKAN: Section untuk menampilkan pengalaman kompetisi user -->
                    <div class="border-t border-gray-100 pt-4 mt-2">
                        <h4 class="text-sm font-medium text-gray-500 mb-2">Competition Experience</h4>
                        <div class="flex flex-wrap gap-2 mb-4">
                            @if(!empty($user->experience) && is_array($user->experience))
                                @foreach($user->experience as $exp)
                                    <!-- #DITAMBAHKAN: Badge kategori dengan highlight untuk kategori yang cocok dengan filter -->
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium {{ $exp == request('category') ? 'bg-green-100 text-green-800 ring-2 ring-green-500' : 'bg-indigo-100 text-indigo-800' }}">
                ...
                <div class="mt-4 p-6 pt-0">
                    <a href="mailto:{{ $user->email }}" class="inline-flex items-center justify-center w-full px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        <i class="bi bi-envelope mr-2"></i> Connect with {{ $firstName }}
                    </a>
                </div>
            </div>
        @endforeach
    ...
    <div class="text-center py-12 bg-white rounded-xl shadow-md mb-10">
        ...
        <!-- #DITAMBAHKAN: Pesan khusus saat tidak ada anggota dengan kategori pengalaman yang dipilih -->
        @if(request('category'))
        ...
        <!-- #DITAMBAHKAN: Pesan default saat tidak ada anggota tim yang ditemukan -->
        @else
    ...
    <!-- Team Building Tips Section: DIPERBARUI - Tampilan tips dengan grid layout dan efek hover -->
