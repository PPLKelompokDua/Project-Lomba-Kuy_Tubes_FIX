      ...
      <div class="p-6">
        ...
        <div class="flex justify-between items-start">
          <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Created {{ $event->created_at->diffForHumans() }}</p>
          </div>
        </div>
        ...
        <div class="mt-6">
          <!-- Feedback Toggle -->
          <button onclick="toggleFeedback('{{ $event->id }}')" class="flex items-center text-indigo-600 hover:text-indigo-800">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
            </svg>
            <span>Feedback ({{ $event->feedbackEvent->count() }})</span>
          </button>

          <!-- Feedback Content (Hidden by default) -->
     <div id="feedback-{{ $event->id }}" class="mt-4 hidden">
            <!-- Feedback Form -->
            <form action="/event/list-my-event/feedback" method="POST" class="mb-6">
              @csrf
              @method('POST')
            ...
            <!-- Feedback List -->
            <div class="space-y-4">
              @forelse($event->feedbackEvent as $feedback)
              <div class="flex space-x-3 group" id="feedback-item-{{ $feedback->id }}">
                ...
                <div class="bg-gray-50 p-4 rounded-lg flex-1 relative">
                  ...
                  <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex space-x-2">
                    ...
                    <!-- Delete Button -->
                    <form action="/event/list-my-event/feedback/{{$feedback->id}}" method="POST" class="inline">
                      @csrf
                      @method('DELETE')
                      <input type="hidden" name="event_id" value="{{ $event->id }}">
                  ...
                  <div id="feedback-edit-form-{{ $feedback->id }}" class="hidden mt-2">
                    <form action="/event/list-my-event/feedback/{{$feedback->id}}" method="POST">
                      @csrf
                      @method('PUT')
                      <input type="hidden" name="event_id" value="{{ $event->id }}">
