...
<div class="max-w-4xl mx-auto py-8">
    <div class="bg-white rounded-2xl shadow-lg p-6 md:p-8 hover-rise" data-aos="fade-up">
        ...
        <div data-aos="fade-up" data-aos-delay="200">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-bold text-gray-800">Competition Bookmarks</h2>
            ...
            <div class="grid sm:grid-cols-2 gap-6">
                @forelse($user->savedCompetitions as $competition)
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden transform transition hover:-translate-y-2 hover:shadow-xl" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                        ...
                        <div class="p-6">
                            <h5 class="text-xl font-bold text-gray-800 mb-2">{{ $competition->title ?? 'Competition Title' }}</h5>
                            <p class="text-sm text-indigo-600 font-semibold mb-2">{{ $competition->category ?? 'General' }}</p>
                            <p class="text-gray-600 text-sm mb-4">{{ \Illuminate\Support\Str::limit($competition->description ?? 'Competition description.', 100) }}</p>
                        </div>
                        <div class="p-6 bg-gray-50 border-t border-gray-100">
                            <div class="flex items-center text-sm mb-2">
                                <i class="fas fa-gift text-indigo-600 mr-2"></i>
                                <span class="text-gray-800 font-semibold">Prize: {{ $competition->prize ?? 'Not specified' }}</span>
                            </div>
                            <div class="flex items-center text-sm mb-4">
                                <i class="fas fa-clock text-red-500 mr-2"></i>
                                <span class="text-gray-800">Deadline: {{ \Carbon\Carbon::parse($competition->deadline)->format('d M Y') }}</span>
                            </div>
                            <a href="#" class="block w-full bg-indigo-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-indigo-700 transition text-center">
                                View Details
                            </a>
                        </div>
                    </div>
                @empty
                    <div class="col-span-2 bg-indigo-50 rounded-xl p-8 text-center" data-aos="fade-up">
                        ...
                        <h3 class="text-lg font-semibold text-gray-800 mb-2">No Saved Competitions</h3>
                        <p class="text-gray-600">Save interesting competitions to revisit them later.</p>
