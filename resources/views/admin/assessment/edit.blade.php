...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <h1 class="text-4xl font-bold mb-6 text-indigo-600" data-aos="fade-up">Edit Assessment Question</h1>
    ...
    <div class="bg-white p-6 rounded-lg shadow hover-rise" data-aos="fade-up" data-aos-delay="100">
        <div class="border-l-4 border-indigo-400 pl-4 mb-6">
            <h2 class="text-xl font-semibold text-indigo-700">Update Question</h2>
        </div>
        ...
        <form method="POST" action="{{ route('admin.assessment-questions.update', $assessment_question->id) }}">
            ...
            <div class="mb-6" data-aos="fade-up" data-aos-delay="150">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="category" id="category" value="{{ old('category', $assessment_question->category) }}" class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg input-dynamic-border focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition" placeholder="Enter category" required>
                    @error('category')
            ...
            <div class="mb-6" data-aos="fade-up" data-aos-delay="200">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="question" id="question" value="{{ old('question', $assessment_question->question) }}" class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg input-dynamic-border focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition" placeholder="Enter question" required>
                    @error('question')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="mb-6" data-aos="fade-up" data-aos-delay="250">
                ...
                <div class="space-y-4">
                    @foreach($assessment_question->options as $opt)
                        <div class="flex gap-3 items-center">
                            <span class="w-6 font-medium text-gray-600">{{ $opt->label }}.</span>
                            <input type="hidden" name="options[{{ $loop->index }}][label]" value="{{ $opt->label }}">
                            <div class="relative flex-1">
                                <i class="fas fa-list-ul absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                                <input type="text" name="options[{{ $loop->index }}][text]" value="{{ old('options.' . $loop->index . '.text', $opt->text) }}" placeholder="Answer {{ $opt->label }} text" class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg input-dynamic-border focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition" required>
